...
def add_rec():
    ...
    try: 
        ...
        c.execute(" insert into  Fabric values(?,?,?,?,?,?,?)",
                (int(FabricIdEN.get()),clicked.get(),float(FabriclentghEN.get()),orderarriveEN.get(),dispatchedEN.get(),float(TotalpaymentEN.get()),selected.get()))
        conn.commit()
        print("RECORD ADDED")
        messagebox.showinfo('SUCCESS','Record added successfully')    
    except:
        print('ERROR IN ADDING RECORD')
        messagebox.showerror('FAILED','Entries should no be blank and LOT-ID should be integer and cannot be repeated')
        conn.rollback()
...
def show_selected_rec(event):
    ...
    #clearing record by calling clear_rec  function
    clear_rec()
     ...
     # Create a database or connection
        ...
        try:
         ...
         #commit function
            ...
            messagebox.showerror("UPDATING RECORD FAILURE","Entries should no be blank and LOT-ID should be integer and cannot be repeated,TRY AGAIN")
            conn.rollback()
        conn.close()
...
def delete_rec():
    ...
    try:
        ...
        print("Error in delete operation")
        messagebox.showerror("FAILED","Record deleted unsuccessfully")
        conn.rollback()
...
def deletePop():
    message = messagebox.askquestion("Delete Confirmation", "Are you sure you want to delete this record?")
    if message == 'yes':
        delete_rec()
...
shippedlist = ['YES','NO','WAITING','PACKED','LOADED','ON THE WAY','DELIVERED']
selected= StringVar()
...
tvfabric.column('#1',width = 90,anchor = 'center',stretch = True)

tvfabric.heading('#2',text = 'FABRIC-TYPE',anchor = 'center')
tvfabric.column('#2',width = 100 ,anchor = 'center',stretch = False)


tvfabric.heading('#3',text = 'FABRIC-LENGTH(CM)',anchor = 'center')
tvfabric.column('#3',width = 125 ,anchor = 'center',stretch = True)
...
#bind the treeview to the function show_selected_record so that we can select one record form many records in the treeview
tvfabric.bind("<<TreeviewSelect>>",show_selected_rec)

#placing the treeview on the window
...
vsb.place(x=1065, y=383, height=263)
